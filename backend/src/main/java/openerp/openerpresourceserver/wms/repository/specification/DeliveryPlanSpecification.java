...
@AllArgsConstructor
@NoArgsConstructor
public class DeliveryPlanSpecification implements Specification<DeliveryPlan> {
    private DeliveryPlanGetListFilter filter;
    @Override
    public Predicate toPredicate(Root<DeliveryPlan> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
        ...
        if (facilityId != null && !facilityId.isEmpty()) {
            predicates.add(criteriaBuilder.equal(root.join("facility").get("id"), facilityId));
        }
