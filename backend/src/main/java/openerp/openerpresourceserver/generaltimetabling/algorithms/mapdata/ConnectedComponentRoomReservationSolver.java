...
public class ConnectedComponentRoomReservationSolver {
    List<GeneralClass> cls;
    int n;
    List<Integer>[] A;// Adjacent list
    boolean[] visited;
    public List<List<GeneralClass>> computeRoomReservationCluster(List<GeneralClass> cls, List<ClassGroup> classGroups){
        this.cls = cls;
        ...
        List<List<GeneralClass>> res = new ArrayList<>();
        for(int i = 0; i < n; i++){
            if(visited[i] == false){
                List<GeneralClass> CC = new ArrayList<>();
                dfs(i,CC);
                res.add(CC);
            }
        }
    ...
    private void dfs(int i, List<GeneralClass> CC){
        visited[i] = true;
        CC.add(cls.get(i));
        for(int j: A[i]){
            if(!visited[j]){
