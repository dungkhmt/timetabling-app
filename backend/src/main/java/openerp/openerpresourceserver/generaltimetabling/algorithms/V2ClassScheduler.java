...
public class V2ClassScheduler {
    ...
    */
    public MapDataScheduleTimeSlotRoomWrapper mapDataNew(List<ModelResponseTimeTablingClass> classes,
        ...
        int n = 0;
        Map<Integer, ModelResponseTimeTablingClass> mClassSegment2Class = new HashMap();
        Map<Integer, TimeTablingClassSegment> mClassSegment2RoomReservation = new HashMap<>();
        for (int i = 0; i < classes.size(); i++) {
        ...
        log.info("mapDataNew DONE build mClassId2ListGroupIds with keys " + mClassId2ListGroupIds.keySet().size());
        ...
        for(ClassGroup cg: classGroups){
            Long classId = cg.getClassId();
            ModelResponseTimeTablingClass gc = mClassId2Class.get(cg.getClassId());
            if(mClassId2PrioritizedClassRooms.get(classId)==null)
                mClassId2PrioritizedClassRooms.put(classId, new ArrayList<>());
            Long groupId = cg.getGroupId();
            Group g = mId2Group.get(groupId);            
            if(g.getPriorityBuilding()!=null){
                ...
                if(mBuilding2ClassRooms.get(buildingId)!=null) {
                    for (Classroom r : mBuilding2ClassRooms.get(buildingId)) {
                        if (gc != null && gc.getQuantityMax() != null && r.getQuantityMax() != null && r.getQuantityMax() >= gc.getQuantityMax())
                            mClassId2PrioritizedClassRooms.get(classId).add(r);
                    }
                }
            }
        }
        log.info("mapDataNew, DONE build mClassId2PrioritizedClassRooms keys = " + mClassId2PrioritizedClassRooms.keySet().size());
        ...
        for (int i = 0; i < classes.size(); i++) {
            ...
            if (gc.getTimeSlots() != null) {
                for (int j = 0; j < gc.getTimeSlots().size(); j++) {
                    ...
                    TimeTablingClassSegment rr = gc.getTimeSlots().get(j);
                    idx++; // new class-segment (RoomReservation)
                    nbSplits[idx] = gc.getTimeSlots().size();
                    if(mClassId2ListGroupIds.get(gc.getId()) != null) {
                        for (Long gId : mClassId2ListGroupIds.get(gc.getId())) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = i+1; j < n; j++) {
                ...
                ModelResponseTimeTablingClass ci = classes.get(indexOfClass[i]);
                ModelResponseTimeTablingClass cj = classes.get(indexOfClass[j]);
                if (ci.getId().equals(cj.getParentClassId()) ||
                        cj.getId().equals(ci.getParentClassId())// lop LT va BT (lop con) conflict
                ) {
                    //conflict.add(new Integer[]{i,j});
    ...
    public List<GeneralClass> autoScheduleTimeSlotRoom(List<GeneralClass> classes,
        ...
        if (!solver.hasSolution()) {
            ...
            for (int i = 0; i < data.getClassSegments().size(); i++) {
                ...
                ModelResponseTimeTablingClass gClass = D.getMClassSegment2Class().get(i);
 ...
 */
    ...
    public List<ModelResponseTimeTablingClass> autoScheduleTimeSlotRoomNew(List<ModelResponseTimeTablingClass> classes,
                                                                           ...
                                                                           int timeLimit, String algorithm,
                                                                           List<TimeTablingConfigParams> params) {
        log.info("autoScheduleTimeSlotRoomNew, START.... classes.sz = " + classes.size());
        //for(int i = 0; i < rooms.size(); i++){
        //    log.info("autoScheduleTimeSlotRoom, room[" + i + "] = " + rooms.get(i).getClassroom());
        //}
